...
public class Jdbc3DML {
	...
	public static void main(String[] args) throws SQLException, ClassNotFoundException {
		...
		/*String [] sorgular= { //bu 3 veriyi tek seferde girebilmek icin bir diziye yani arraye atiyoruz
		...
		int count=0;
		for (String each : sorgular) {//her eleman icin st.executeUpdate ile donerek teker teker yazdir
			st.executeUpdate(each);
		count++;// kac veri eklendigini yani dongunun kac kez calistigini ogrenebilmek icin...
		}
		System.out.println(count+" veriler eklendi"); //bu syso verilerin kayit edilip edilmedigi icindir. 
												//bu sorgu calisirsa demekki veriler eklendi*/
		...
		for (String each: sorgular1) {
			...
			st.addBatch(each);//for dongusuyle arrayden gelen verileri bir araya toplar
		}
		st.executeBatch();//topladigi verileri bir seferde database gonderir.
			//sql e gidip commit; yazip run ettikten sonra calistirip gorebiliriz...
	}
